...
| `fitness/` | 训记 (Xunji) App body / training / diet data via Open API | `data/sync/cook-diet-sync.json` (cache is under `data/cache/`) |
...
> **`fitness/` has no web dashboard** — it's a pure Python (stdlib-only) Open API client for the 训记 App. Its `skills/*.md` files are **empty placeholders** and `scripts/xunjiapi.py` ships **no API keys**: request your keys in the 训记 App, paste the returned Skill docs into `skills/*.md`, and inject keys via the `XUNJI_TRAIN_KEY` / `XUNJI_BODY_KEY` / `XUNJI_DIET_KEY` / `XUNJI_SEARCH_KEY` / `XUNJI_TPL_KEY` environment variables.
